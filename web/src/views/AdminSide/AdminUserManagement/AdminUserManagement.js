...
export default {
    ...
    methods: {
        ...
        async _getListWorklogsOfUser(params){
            const response = await AdminUserManagementService.getUserWorklogs(params)
            if (!response) {
                this.$router.push('/user/login');
                return;
            }
        ...
        async _getHistoryActivityOfSpecificUser(params){
            const response = await AdminUserManagementService.getUserWorkHistory(params)
            if (!response) {
                this.$router.push('/user/login');
                return;
            }
        ...
        async _getSpecificEmployeeInfo(params){
            ...
            const response = await AdminUserManagementService.getUserInfo(params)
            if (!response) {
                this.$router.push('/user/login');
                return;
            }
        ...
        async _getSpecificProjectUserJoined(params){
            const response = await AdminUserManagementService.getUserProjectJoined(params)
            if (!response) {
                this.$router.push('/user/login');
                return;
            }
